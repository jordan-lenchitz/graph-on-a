...
# passive origin clusters = google cloud run
...
# edge compute interception = cloudflare workers 
...
# distributed state management = cloudflare workers kv
...
1. a client request enters the global network and is immediately captured by the edge proxy
2. the edge proxy performs a low-latency read against the distributed kv store to retrieve the current `active_region` identifier
3. the proxy maps the retrieved identifier against an internal, pre-defined topology map to determine the physical url of the active origin cluster
4. the request is transparently forwarded to the resolved origin url and response proxied back through the edge to the client
